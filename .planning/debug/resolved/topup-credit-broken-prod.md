...
root_cause: Two issues: (1) /billing/plans was returning 500 for new users (fixed by earlier dev→prod merge on Mar 5). (2) No dedicated topup callback route — initiateTopupPayment stored pending data only in SESSION, but webhook handler looked for DB record by invoice ID and found nothing. Credits were NEVER added after successful topup payment.

fix: Added handleTopupCallback() method to PaymentController mirroring the working handleTrialCallback pattern. Changed topup callback_url from billing.webhook to billing.topup.callback. Added GET route /billing/topup/callback. Created pending TopupPurchase DB record before redirect (so record exists for webhook). On callback: verifies paymentId with MyFatoorah, reads session pending_topup, marks DB record completed, increments user credits.

verification: Deployed to prod (commit b48ebcf). Route verified live via artisan route:list. No new errors in prod log after deploy.
files_changed:
